...
export const ProgramSlice = createSlice({
  ...
  initialState,
  reducers: {
    saveProgramSlice(state, action) {
      state.programCode = action.payload.data.program_code;
    ...
    searchProgram: (state, action) => {
      /// sử dụng filter để lấy ra program code giống với những gì nhập vào
      const filteredUsers = state.programList.filter((list) =>
        toLowerCaseNonAccentVietnamese(list.program_code + list.program_name)
          .toUpperCase()
          .includes(
      ...
      return {
        ...state,
        filteredUsers:
          action.payload.length > 0 ? filteredUsers : [...state.programList],
      };
    },
